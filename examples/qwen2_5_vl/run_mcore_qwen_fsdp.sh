...
MEGATRON_PATH=/workspace/mcore-cogvidoex/
PAI_PATH=$( dirname $( dirname ${CURRENT_DIR}))
export PYTHONPATH=${PAI_PATH}:${MEGATRON_PATH}:$PYTHONPATH
...
"
    ...
    -o ${OUTPUT_BASEPATH}/profile/${NAME}_${current_time} \
    --force-overwrite true \
    --capture-range=cudaProfilerApi \
    --capture-range-end=stop
    "
elif [ $PROFILE = false ]; then
    profile_options=" \
        "
    profile_cli=""
fi
...
if [ $WANDB = true ]; then
    ...
    --wandb-exp-name qwen2_5_vl_${MODEL_SIZE}_${PR}_tp${TP}_pp${PP}_cp${CP}_ac${AC}_do${DO}_sp${SP}_ti${TRAIN_ITERS}_wi${LR_WARMUP_ITERS} \
...
megatron_options="  \
        ...
        ${wandb_options}
        "
run_cmd="${profile_cli} torchrun $DISTRIBUTED_ARGS pretrain_qwen.py
 ${megatron_options} ${dataset_option} ${pr_options} ${load_options} ${activation_checkpoint_options} ${fsdp_options} \
